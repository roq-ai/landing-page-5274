...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createBusiness() {
    ...
    if (body?.marketing_strategy?.length > 0) {
      const create_marketing_strategy = body.marketing_strategy;
      body.marketing_strategy = {
        create: create_marketing_strategy,
      };
    } else {
      delete body.marketing_strategy;
    }
